...
``test_pn7150.py`` is written to run on a board (copy it to ``code.py``), where
...
def test_on_device_suite_passes_on_host(capsys):
    source = io.open(os.path.join(REPO, "test_pn7150.py"), encoding="utf-8").read()
    namespace = {"__name__": "__main__"}
    exec(compile(source, "test_pn7150.py", "exec"), namespace)  # noqa: S102
